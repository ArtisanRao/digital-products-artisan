...
const UI_VERSION = "product-hardened-v4";
...
/** Strip any non-plain stuff to avoid surprises during RSC render */
function sanitizeProduct(raw: any) {
  ...
  // Price: allow number or string only
  safe.price =
    typeof raw.price === "number" || typeof raw.price === "string" ? raw.price : "";
  // Images: strings only
  if (Array.isArray(raw.images)) {
    safe.images = raw.images.filter((x: any) => typeof x === "string" && x.trim());
  } else {
    safe.images = [];
  }
...
/* --------- metadata with no data access (prevent build crashes) --------- */
...
export default async function ProductPage({
  ...
  // Resolve product (do not catch notFound/redirect)
  const raw = byParam(String(slug));
  if (!raw) return notFound();

  const target = canonicalHref(raw).toLowerCase();
  const current = `/products/${encodeURIComponent(String(slug))}`.toLowerCase();
  if (current !== target) {
    redirect(target);
  }

  // From here on, never throw — render a safe UI even if data is odd
  try {
    ...
    // Log on the server, but do not crash the page
    console.error("ProductPage render error:", { slug, err });
    const safeTitle = canonicalSlug(raw).replace(/-/g, " ").replace(/\b\w/g, (m) => m.toUpperCase());
    const imgs = productImages(raw);
    return (
      <main className="container mx-auto px-4 py-10">
        <h1 className="text-3xl md:text-4xl font-bold">{safeTitle || "Product"}</h1>
        <p className="mt-2 text-sm text-red-600">
          We couldn’t load some details for this product, but you can still view the basics below.
        </p>
        <div className="mt-6 rounded-xl border bg-white p-2 max-w-xl">
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img
            src={(imgs[0] ?? "/images/placeholder.jpg") as string}
            alt={safeTitle || "Product"}
            className="w-full h-auto object-contain"
            loading="eager"
          />
        </div>
